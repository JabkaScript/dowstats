...
defineRouteMeta({
  openAPI: {
   description: 'Players ladder by MMR with support for name search, mod and rating type (solo/team) selection, sorting, server filter, and pagination. Each item also includes the player\'s favorite race (favoriteRaceId, favoriteRaceName, favoriteRaceShortName), total games, wins and winrate.',
     parameters: [
       ...
       { in: 'query', name: 'minGames', required: false, description: 'Minimum number of games not less than this value (>=), default 1. For solo uses 1X1X columns, for team uses NxNX columns.', schema: { type: 'integer', minimum: 0 } },
    ],
  },
})
...
export default defineEventHandler(async (event) => {
  ...
  const totalRows = await db
    ...
    .innerJoin(
      tables.players,
      eq(tables.players.id, tables.playersStats.playerId)
    )
  ...
  if (mmrType === 'solo') {
    ...
    rows = (await db
      .select({
        ...
        avatarUrl: tables.players.avatarUrl,
        mmr: mmrCol,
        a1: tables.playersStats['2X21'], a2: tables.playersStats['2X22'], a3: tables.playersStats['2X23'], a4: tables.playersStats['2X24'], a5: tables.playersStats['2X25'], a6: tables.playersStats['2X26'], a7: tables.playersStats['2X27'], a8: tables.playersStats['2X28'], a9: tables.playersStats['2X29'],
        b1: tables.playersStats['3X31'], b2: tables.playersStats['3X32'], b3: tables.playersStats['3X33'], b4: tables.playersStats['3X34'], b5: tables.playersStats['3X35'], b6: tables.playersStats['3X36'], b7: tables.playersStats['3X37'], b8: tables.playersStats['3X38'], b9: tables.playersStats['3X39'],
        c1: tables.playersStats['4X41'], c2: tables.playersStats['4X42'], c3: tables.playersStats['4X43'], c4: tables.playersStats['4X44'], c5: tables.playersStats['4X45'], c6: tables.playersStats['4X46'], c7: tables.playersStats['4X47'], c8: tables.playersStats['4X48'], c9: tables.playersStats['4X49'],
      })
      .from(tables.playersStats)
      .innerJoin(tables.players, eq(tables.players.id, tables.playersStats.playerId))
      .where(and(...conditions))
  ...
  if (rows.length > 0) {
    ...
    const distinctRaceIds = Array.from(new Set(Array.from(favoriteByPlayer.values()).map(v => v.raceId)))
  ...
  if (rows.length > 0) {
    const totals = await db
      .select({ playerId: tables.playersStats.playerId, games: totalGamesExpr, wins: totalWinsExpr })
      .from(tables.playersStats)
      .where(and(
        eq(tables.playersStats.modId, modId),
        eq(tables.playersStats.seasonId, seasonId),
        inArray(tables.playersStats.playerId, rows.map(r => Number(r.playerId)))
      ))
