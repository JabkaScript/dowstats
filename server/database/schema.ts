import { mysqlTable, primaryKey, varchar, int, index, text, datetime, unique, date, bigint, timestamp, float, tinyint } from "drizzle-orm/mysql-core"


export const banType = mysqlTable("ban_type", {
...
(table) => [
	primaryKey({ columns: [table.banType], name: "ban_type_ban_type"}),
]);

export const gameRaitings = mysqlTable("game_raitings", {
	id: int().notNull(),
	gameId: int("game_id").notNull(),
	lowestMmr: int("lowest_mmr").notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "game_raitings_id"}),
]);
...
(table) => [
	primaryKey({ columns: [table.id], name: "maps_id"}),
]);

export const mapsCache = mysqlTable("maps_cache", {
	id: int().autoincrement().notNull(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "maps_cache_id"}),
]);

export const mapsracesCache = mysqlTable("mapsraces_cache", {
	id: int().autoincrement().notNull(),
	mapid: int(),
...
export const minMaxRanks = mysqlTable("min_max_ranks", {
...
(table) => [
	primaryKey({ columns: [table.id], name: "mods_id"}),
]);

export const modsRaces = mysqlTable("mods_races", {
	modId: int("mod_id").notNull().references(() => mods.id),
	raceId: int("race_id").notNull().references(() => races.id),
	id: bigint({ mode: "number" }).autoincrement().notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "mods_races_id"}),
]);

export const playerMapStats = mysqlTable("player_map_stats", {
	id: int().autoincrement().notNull(),
...
(table) => [
	...
	index("idx_map_stats_lookup").on(table.mapName, table.gameType, table.raceId, table.modId, table.seasonId),
	index("idx_mod_season").on(table.modId, table.seasonId),
	index("idx_player_id").on(table.playerId),
	index("idx_player_stats_lookup").on(table.playerId, table.gameType, table.raceId, table.modId, table.seasonId, table.totalGames),
	...
	unique("unique_player_map_stats").on(table.playerId, table.mapName, table.gameType, table.raceId, table.modId, table.seasonId),
]);

export const playerSkill = mysqlTable("player_skill", {
	id: int().autoincrement().notNull(),
	name: text().notNull(),
},
(table) => [
	primaryKey({ columns: [table.id], name: "player_skill_id"}),
]);
...
export const playersBanned = mysqlTable("players_banned", {
	id: int().autoincrement().notNull(),
	...
	dateEnd: date("date_end", { mode: 'string' }),
	reason: text(),
	banType: varchar("ban_type", { length: 32 }).default('cheater').references(() => banType.banType),
},
(table) => [
...
export const playersPd = mysqlTable("players_pd", {
	id: int().autoincrement().notNull(),
...
export const playersStats = mysqlTable("players_stats", {
	id: int().autoincrement().notNull(),
	playerId: int("player_id").notNull(),
	seasonId: int("season_id").notNull(),
	modId: int("mod_id").notNull(),
...
export const racesCache = mysqlTable("races_cache", {
	id: int().autoincrement().notNull(),
	server: varchar({ length: 128 }),
	patch: varchar({ length: 128 }),
	race1Pick: int("race_1_pick"),
...
export const seasonModRacesWinrate = mysqlTable("season_mod_races_winrate", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	server: int().notNull().references(() => servers.id, { onUpdate: "cascade" } ),
	season: int().notNull().references(() => seasons.id, { onUpdate: "cascade" } ),
	mod: int().notNull().references(() => mods.id, { onUpdate: "cascade" } ),
	race: int().notNull().references(() => races.id, { onUpdate: "cascade" } ),
...
export const steamIdToUpdate = mysqlTable("steam_id_to_update", {
	steamId: bigint("steam_id", { mode: "number" }).notNull(),
},
(table) => [
	unique("steam_id").on(table.steamId),
]);
...
export const urlLogs = mysqlTable("url_logs", {
	id: int().autoincrement().notNull(),
	url: text(),
...
(table) => [
	primaryKey({ columns: [table.id], name: "url_logs_id"}),
]);
